...
func get_dialogue(sequence_number: int, type: String) -> Array[String]:
	var response: String = DialogueLines.reactions[DialogueLines.persons[candidate_id]][_phases[sequence_number]][type].pick_random()
	return [response]
...
func get_options(sequence_number: int) -> Array:
	...
	var self_options = DialogueLines.options[DialogueLines.persons[candidate_id]][_phases[sequence_number]]
	...
	for i in range(3):
		var other_options = DialogueLines.options[other_persons.pick_random()][_phases[sequence_number]]
		result.append(
			{
